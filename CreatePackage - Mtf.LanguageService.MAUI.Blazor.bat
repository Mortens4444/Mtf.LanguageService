...
set "ProjectName=Mtf.LanguageService.MAUI.Blazor"
set "TargetDir=C:\NuGetTest"

for %%F in ("%TargetDir%\Mtf.MAUI..Blazor.LanguageService.*.nupkg") do (
    set "FileName=%%~nxF"
...
for %%F in ("%TargetDir%\Mtf.MAUI.Blazor.LanguageService.*.nupkg") do call :ProcessFile "%%F"
goto :UpdatePackages
...
call set "RestPart=%%FileName:Mtf.MAUI.Blazor.LanguageService.=%%"
