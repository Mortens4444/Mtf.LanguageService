...
set "ProjectName=Mtf.LanguageService.MAUI"
set "TargetDir=C:\NuGetTest"

for %%F in ("%TargetDir%\Mtf.MAUI.LanguageService.*.nupkg") do (
    set "FileName=%%~nxF"
...
for %%F in ("%TargetDir%\Mtf.MAUI.LanguageService.*.nupkg") do call :ProcessFile "%%F"
goto :UpdatePackages
...
call set "RestPart=%%FileName:Mtf.MAUI.LanguageService.=%%"
